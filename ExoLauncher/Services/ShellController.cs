...
/// <summary>
/// Typed host surface for the WinUI shell. Discovery, install/progress, launch,
/// and deps stay here; the shell owns pixels.
/// </summary>
public sealed class ShellController
{
    ...
    private readonly LibraryUpdatedPublisher _libraryUpdated;
    ...
    public event EventHandler<AchievementSnapshot>? AchievementsUpdated;
    public event EventHandler<LaunchStatusInfo>? LaunchStatusChanged;
    public event EventHandler<SearchPartialEventArgs>? SearchPartial;

    public ShellController(AppServices services, DispatcherQueue queue)
    {
        _services = services;
        _queue = queue;
    ...
    public void Detach()
    {
        ...
        search?.Dispose();
        try { _libraryUpdated.Dispose(); } catch { }
    }

    public Task<object> GetLibraryAsync(bool force = false) =>
        LibraryGetAsync(P(new { force }), hasParams: true, force);

    public Task<object> RefreshLibraryAsync() =>
        LibraryGetAsync(P(new { force = true }), hasParams: true, force: true);

    public Task<object> GetGameAsync(string id) =>
        GameGetAsync(P(new { id }), hasParams: true);

    public Task<object> LaunchAsync(string id, bool skipDeps = false) =>
        GameLaunchAsync(P(new { id, skipDeps }), hasParams: true);

    public Task<object> StopAsync(string id) =>
        GameStopAsync(P(new { id }), hasParams: true);

    public Task<object> InstallAsync(string id, string? path = null, bool skipDeps = false, string? title = null) =>
        GameInstallAsync(P(new { id, path, skipDeps, title }), hasParams: true);

    public Task<object> UpdateAsync(string id) =>
        GameUpdateAsync(P(new { id }), hasParams: true);

    public Task<object> UninstallAsync(string id) =>
        GameUninstallAsync(P(new { id }), hasParams: true);

    public Task<object> RepairAsync(string id) =>
        GameRepairAsync(P(new { id }), hasParams: true);

    public object Extras(string id) =>
        GameExtras(P(new { id }), hasParams: true);

    public object OpenFolder(string id) =>
        GameOpenFolder(P(new { id }), hasParams: true);

    public object ToggleFavorite(string id) =>
        GameToggleFavorite(P(new { id }), hasParams: true);

    public object CancelInstall() => _services.Launcher.Cancel();

    public object CurrentProgress() => MapProgress(_services.Launcher.CurrentProgress);

    public object AchievementsGet(string id) =>
        AchievementGet(P(new { id }), hasParams: true);

    public Task<object> AchievementsRefreshAsync(string id) =>
        AchievementRefreshAsync(P(new { id }), hasParams: true);

    public Task<object> StoresAuthAsync(string store) =>
        StoresAuthAsync(P(new { store }), hasParams: true);

    public Task<object> SearchAsync(string query) =>
        StoresSearchAsync(P(new { query }), hasParams: true);

    public object DepsList() => ListDependencies();

    public object DepsOfferInstall(string id) =>
        DepsOfferInstall(P(new { id }), hasParams: true);

    public object StoreMatrix() => _services.Library.StoreMatrix();

    public object GetSettings() => BuildSettings();

    public object SetSettings(object patch) =>
        SetSettings(P(patch), hasParams: true);

    public object PreviewTrophy(string? gameTitle = null, string? unlockName = null, string? gameId = null) =>
        PreviewTrophyNotification(P(new { gameTitle, unlockName, gameId }), hasParams: true);

    public object Minimize() => HideToNotificationArea();

    public object ToggleMaximize() => MaximizeWindow();

    public object WindowState() => CurrentWindowState();

    public object Close() => CloseWindow();

    public object OpenUrl(string url) =>
        OpenUrl(P(new { url }), hasParams: true);

    public object OpenPath(string path) =>
        OpenPath(P(new { path }), hasParams: true);

    public Task<object> ShowStoreAsync(string store) =>
        ShowStoreAsync(P(new { store }), hasParams: true);

    public Task<object> PickFolderAsync(string? title = null) =>
        PickFolderAsync(P(new { title }), hasParams: true);

    public Task<object> CheckUpdateAsync() => CheckAppUpdateAsync();

    public Task<object> InstallUpdateAsync() => InstallAppUpdateAsync();

    public Task<object> DlssStatusAsync(string? id = null) =>
        DlssStatusAsync(P(new { id }), hasParams: true);

    public Task<object> DlssUpdateAllAsync(string id) =>
        DlssUpdateAllAsync(P(new { id }), hasParams: true);

    public Task<object> DlssEnsureLatestAsync() => EnsureLatestDlssAsync();

    public Task<object> DlssRestoreAsync(string id) =>
        DlssRestoreAsync(P(new { id }), hasParams: true);

    public object AppVersion() => new { version = _services.AppVersion };

    public (bool IsRunning, bool CanStop) RunState(GameEntry game, bool discoverExternal = false)
    {
        var state = _services.Launcher.GetGameRunState(game, discoverExternal);
        return (state.IsRunning, state.CanStop);
    }

    private void OnProgress(InstallProgress p) =>
        RaiseOnQueue(() => ProgressChanged?.Invoke(this, p));

    private void OnGameSessionCompleted(GameEntry game) =>
        RaiseOnQueue(() => LaunchStatusChanged?.Invoke(this, new LaunchStatusInfo
        {
            gameId = game.Id,
            Ok = true,
            Message = "Game closed.",
    ...
    private void OnAchievementSnapshotUpdated(AchievementSnapshot snapshot) =>
        RaiseOnQueue(() => AchievementsUpdated?.Invoke(this, snapshot));

    private void OnLibraryUpdated()
    {
        try { _libraryUpdated.Request(); }
        catch (Exception ex) { AppLog.Debug("library.updated schedule failed: " + ex.Message); }
    }

    private Task PublishLibrarySnapshotAsync(CancellationToken ct)
    {
        _ = ct;
        try
        {
            ...
            if (games.Count == 0)
                games = _services.Library.PeekCachedLibrary();
            PostLibrarySnapshot(games);
        }
        catch (ObjectDisposedException ex)
        {
            AppLog.Debug("library.updated failed: " + ex.Message);
            try { PostLibrarySnapshot(_services.Library.PeekCachedLibrary()); }
            catch { /* last-good already painted */ }
        }
        catch (Exception ex)
        {
            AppLog.Debug("library.updated failed: " + ex.Message);
    ...
    private void PostLibrarySnapshot(IReadOnlyList<GameEntry> games)
    {
        var settings = _services.Settings.Current;
        RaiseOnQueue(() => LibraryChanged?.Invoke(this, new LibraryChangedEventArgs
        {
            Games = games,
    ...
    private async Task<object> LibraryGetAsync(JsonElement p, bool hasParams, bool force = false)
    {
        ...
        try
        {
            ...
            return LibraryPayload(games);
        }
        catch (OperationCanceledException)
        {
            return LibraryPayload(_services.Library.PeekCachedLibrary());
        }
    ...
    private object LibraryPayload(IReadOnlyList<GameEntry> games)
    {
        var settings = _services.Settings.Current;
        return new
        {
    ...
    private GameEntry? TryLibraryOwnedSource(string gameId)
    {
        foreach (var game in _services.Library.PeekCachedLibrary())
        {
            if (string.Equals(game.Id, gameId, StringComparison.OrdinalIgnoreCase) &&
                ...
                (game.Owned || game.CanInstall))
                return game;
            var variant = game.Variants.FirstOrDefault(item =>
                string.Equals(item.Id, gameId, StringComparison.OrdinalIgnoreCase));
            if (variant is not null &&
                variant.EntitlementState is not (EntitlementState.NotOwned or EntitlementState.Unverified) &&
                (variant.Owned || variant.CanInstall))
    ...
    private async Task<object> StoresSearchAsync(JsonElement p, bool hasParams)
    {
        ...
        void PublishPartial(IReadOnlyList<StoreSearchHit> hits)
        {
            try
            {
                var live = LiveSearchHits(hits);
                WarmSearchCovers(query, live, ct);
                RaiseSearchPartial(query, live);
            }
            catch (Exception ex)
            {
        ...
        try
        {
            ...
            var hits = LiveSearchHits(await _services.StoreSearch
    ...
    private static IReadOnlyList<StoreSearchHit> LiveSearchHits(IReadOnlyList<StoreSearchHit> hits) =>
        hits.Where(StoreSearchService.IsLiveSearchHit).ToList();
    ...
    private static object MapSearchHit(StoreSearchHit h)
    {
        var entry = SearchHitEntry(h);
        if (h.Store == StoreKind.Epic)
            entry = EpicPlaytime.Apply([entry], EpicPlaytime.GetCachedMinutes())[0];
        ...
        return new
        {
            ...
            coverUrl = CoverArtService.IsUiLoadableCoverUrl(h.CoverUrl)
                ? h.CoverUrl
                : CoverArtService.ResolvePreferredUrl(entry)
                  ?? CoverArtService.ProvisionalStorePosterUrl(entry),
            coverSource = h.CoverSource,
            owned = h.Owned,
            installed = h.Installed,
            canInstall = h.CanInstall,
    ...
    private async Task<object> GameUninstallAsync(JsonElement p, bool hasParams)
    {
        ...
        if (result.Ok && !result.Queued)
        {
            _services.Library.ForgetInstalled(game.Id);
            _services.Library.Invalidate();
        }
        return new { ok = result.Ok, queued = result.Queued, message = result.Message };
    }
    ...
    private async Task<object> DlssStatusAsync(JsonElement p, bool hasParams)
    {
        ...
        var sdk = await _dlss.GetSdkStatusAsync(CancellationToken.None).ConfigureAwait(true);
        return new
        {
            ok = sdk.Ok,
            cachedVersion = sdk.CachedVersion,
            latestVersion = sdk.LatestVersion,
            alreadyBest = sdk.AlreadyBest,
            message = sdk.Message,
            items = Array.Empty<object>(),
        };
    }

    private async Task<object> EnsureLatestDlssAsync()
    {
        var result = await _dlss.EnsureLatestSdkAsync(CancellationToken.None).ConfigureAwait(true);
        return new
        {
            ok = result.Ok,
            cachedVersion = result.CachedVersion,
            latestVersion = result.LatestVersion,
            alreadyBest = result.AlreadyBest,
            message = result.Message,
        };
    }
    ...
    private object ListDependencies() => new
    {
    ...
    private object PreviewTrophyNotification(JsonElement p, bool hasParams)
    {
        var gameTitle = ReadString(p, hasParams, "gameTitle");
        var unlockName = ReadString(p, hasParams, "unlockName");
        ...
        if (!string.IsNullOrWhiteSpace(gameId))
        {
            ...
            if (game is not null)
            {
                if (string.IsNullOrWhiteSpace(gameTitle)) gameTitle = game.Title;
                coverUrl = CoverArtService.TryResolveLocalFile(game.CoverUrl) ?? game.CoverUrl;
            }
        }
        ...
        void Show()
        {
            try
            {
                if (!_services.TrophyNotifications.Preview(gameTitle, unlockName, coverUrl, () => painted = true))
                    return;
            }
        ...
        else
        {
            ...
            if (!_queue.TryEnqueue(() =>
            {
                ...
                return new { ok = false, message = "Could not show preview." };
            if (!done.Wait(TimeSpan.FromSeconds(2)))
                return new { ok = false, message = "Could not show preview." };
        }

        if (error is not null)
            return new { ok = false, message = error.Message };
        return painted
            ? new { ok = true, message = "Preview shown." }
            : new { ok = false, message = "Unlock notification is not ready." };
    }
    ...
    private object CurrentWindowState()
    {
        var maximized = false;
    ...
    private object OpenUrl(JsonElement p, bool hasParams)
    {
        ...
        if (string.IsNullOrWhiteSpace(url)) return new { ok = false, message = "Missing link." };
        if (!Uri.TryCreate(url, UriKind.Absolute, out var uri)) return new { ok = false, message = "That link is not valid." };
        ...
        if (uri.Scheme is not ("https" or "http" or "steam")) return new { ok = false, message = "That link is not allowed." };
        try
        {
            ...
            return new { ok = true, message = "Opened in your browser." };
        }
        catch (Exception ex)
        {
            return new { ok = false, message = ex.Message };
    ...
    /// <summary>Open a verified installed store client. Does not imply library or game control.</summary>
    ...
    private object OpenOfficialClient(string adapterId, StoreKind kind, string missing)
    {
        ...
        return adapter is null
            ? new { ok = false, message = "Official client is unavailable." }
    ...
    private static string? ResolveGalaxyExe() => GogAdapter.ResolveGalaxy();
    ...
    private async Task<object> InstallAppUpdateAsync()
    {
        var progress = new Progress<(string status, double percent)>(_ => { });
        ...
        if (result.ShouldExit)
        {
            ...
            // settings, sync state, tray resources, and shell services before
    ...
    private object MapGame(GameEntry g, bool discoverExternalRunningGame = false)
    {
        ...
        canRepair = _services.FindAdapter(g.Store) is IStoreRepair repair && repair.CanRepair(g),
        };
    }
    ...
    private object MapProgress(InstallProgress p) => new
    {
        gameId = p.GameId,
    ...
    private static JsonElement P(object value)
    {
    ...
    private void RaiseSearchPartial(string query, IReadOnlyList<StoreSearchHit> hits) =>
        RaiseOnQueue(() => SearchPartial?.Invoke(this, new SearchPartialEventArgs
        {
            Query = query,
            Hits = hits,
        }));

    private void RaiseOnQueue(Action action)
    {
        void Run()
        {
            try { action(); } catch { }
